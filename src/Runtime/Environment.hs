...
importEnv name val envA envB =
  let Environment { variables = variables } = envA
  in  envA { variables = (variables `Map.union` (Map.singleton name val)) }
...
setTypesState e env = env { stypes = e }
...
shadowEnv envA envB =
  let
    Environment { nextFreeRef = nextFreeRef, refs = refs, nextFreeNameId = nextFreeNameId }
      = envB
  in  envA { nextFreeRef    = nextFreeRef
           , refs           = refs
           , nextFreeNameId = nextFreeNameId
           }
...
allocIdent env =
  let Environment { nextFreeNameId = nextFreeNameId } = env
  in  ( (Ident $ "__@$internal_variable__" ++ (show nextFreeNameId) ++ "_")
      , (env { nextFreeNameId = nextFreeNameId + 1 })
      )

allocRef :: Environment -> (Integer, Environment)
allocRef env =
  let Environment { nextFreeRef = freeRef } = env
  in  (freeRef, (env { nextFreeRef = freeRef + 1 }))

setNativeUntypedVariable
  :: (PackableValue a) => String -> a -> Exec (RuntimeValue, Environment)
...
setVariable name val env =
  let Environment { variables = oldVariables } =
          delVariable name $ delDef name env
  in  env { variables = (Map.insert name val oldVariables) }

setVariables :: [Ident] -> [RuntimeValue] -> Environment -> Environment
setVariables names vals env =
  foldl (\env (n, v) -> setVariable n v env) env (zip names vals)
...
findRecordDefByFieldNames (name : _) env =
  let Environment { defs = defs } = env
  in  case Map.elems (Map.filter (defContainsField name) defs) of
        []      -> DInvalid
        (h : t) -> h
...
getVariable name env =
  let Environment { variables = oldVariables } = env
  in  case Map.findWithDefault RInvalid name oldVariables of
...
execFunction
  :: RFunSig -> RFunBody -> [RuntimeValue] -> Exec (RuntimeValue, Environment)
...
callFunctionF
  ...
  -> [RuntimeValue]
  -> Environment
  -> Exec (RuntimeValue, Environment)
callFunctionF (RfFun (RFunVararg) body) args env =
  let argsInCount = length args
  in  callFunctionF (RfFun (RFunSig argsInCount) body) args env
callFunctionF (RfFun (RFunSig argsCount) body) args env =
  let argsInCount = length args
  in  if (argsCount > 0) && (argsInCount < argsCount)
        then
          let fnBody = \paramArgs ->
                execFunction (RFunSig argsCount) body (args ++ paramArgs)
          in  shadow env $ local (\_ -> env) $ return $ newFunction
                (RFunSig (argsCount - argsInCount))
                fnBody
                env
        else if argsInCount > argsCount
          then do
            (val, valEnv) <- execFunction (RFunSig argsCount)
                                          body
                                          (take argsCount args)
            fnCont        <- return $ getRefStorage val valEnv
            (val, valEnv) <- local (\_ -> valEnv) $ callFunctionF
              fnCont
              (drop (argsInCount - argsCount) args)
              valEnv
            return (val, valEnv)
          else do
            (val, valEnv) <- shadow env $ local (\_ -> env) $ execFunction
              (RFunSig argsCount)
              body
              args
            return (val, valEnv)
...
callFunction
  :: Ident -> [RuntimeValue] -> Environment -> Exec (RuntimeValue, Environment)
...
newFunction sig body env =
  let (fr, frEnv) = allocRef env
  in  ((RRef fr), (setRefStorage fr (RfFun sig body) frEnv))

createFunction
  :: Ident
  -> Maybe (Integer, Environment)
  -> RFunSig
  -> RFunBody
  -> Environment
  -> Environment
createFunction name (Just newRef) sig body env =
  let (fr, frEnv) = newRef
  in  setVariable name (RRef fr) $ setRefStorage
        fr
        (RfFun
          sig
          (\args -> do
            local (\e -> setVariable name (RRef fr) e) $ body args
          )
        )
        frEnv
...
callOperator
  :: Ident -> Integer -> [RuntimeValue] -> Exec (RuntimeValue, Environment)
callOperator name@(Ident nameStr) priority args = do
  ...
  case getDef name env of
    ...
    (DOperator opName opPrio opBody) ->
      if (opName == name && opPrio == priority)
        then callFunctionF (RfFun (RFunSig $ length args) opBody) args env
...
callOperatorDS
  :: RuntimeValue -> RuntimeValue -> Exec (RuntimeValue, Environment)
...
createOperatorFunction
  :: Ident -> Integer -> RFunSig -> RFunBody -> Exec (RuntimeValue, Environment)
...
createOperator
  :: OperatorAny -> RFunSig -> RFunBody -> Exec (RuntimeValue, Environment)
createOperator (OperatorAnyF (OperatorF name)) sig body =
  createOperatorFunction (Ident name) 5 sig body
createOperator (OperatorAnyE (OperatorE name)) sig body =
  createOperatorFunction (Ident name) 4 sig body
createOperator (OperatorAnyDS OperatorDS) sig body =
  createOperatorFunction (Ident "*") 3 sig body
createOperator (OperatorAnyD (OperatorD name)) sig body =
  createOperatorFunction (Ident name) 3 sig body
createOperator (OperatorAnyC (OperatorC name)) sig body =
  createOperatorFunction (Ident name) 2 sig body
createOperator (OperatorAnyB (OperatorB name)) sig body =
  createOperatorFunction (Ident name) 1 sig body
createOperator (OperatorAnyA (OperatorA name)) sig body =
  createOperatorFunction (Ident name) 0 sig body
