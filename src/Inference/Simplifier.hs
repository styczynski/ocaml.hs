...
simplifyPattern fn recMode (PatConstr (Ident nameStr) pat) letExpr expr = do
  addExprAnnot $ simplifyPattern
    ...
    recMode
    pat
    (SimplifiedCall (SimplifiedVariable $ Ident $ nameStr ++ "_reverse") letExpr
    )
    expr
simplifyPattern fn recMode ast@(PatList (PList list)) letExpr expr = do
  markTrace ast
  (simpl, _) <- foldrM
    (\(PListElement pat) (expr, n) -> do
      patSimpl <- simplifyPattern fn
                                  recMode
                                  pat
                                  (SimplifiedUnaryOp OpListNth letExpr)
                                  expr
      return (patSimpl, n + 1)
    )
    (expr, 0)
...
simplifyPattern _ _ (PatConst const) letExpr expr = do
  ...
  addExprAnnot $ return $ SimplifiedLet
    id
    (SimplifiedCheck letExpr $ geTypeStaticstScheme const)
    expr
...
simplifyPattern _ True (PatIdent name) letExpr expr =
  addExprAnnot $ return $ SimplifiedLet
    name
    (SimplifiedFixPoint (SimplifiedFunction name letExpr))
    expr
simplifyPattern fn recMode ast@(PatCons hPat tPat) letExpr expr = do
  markTrace ast
  hSimpl <- simplifyPattern fn
                            recMode
                            hPat
                            (SimplifiedUnaryOp OpHead letExpr)
                            expr
  tSimpl <- simplifyPattern fn
                            recMode
                            tPat
                            (SimplifiedUnaryOp OpTails letExpr)
                            hSimpl
  unmarkTrace ast
...
simplifyPattern fn recMode ast@(PatTuple (PTuple el restEls)) letExpr expr = do
  ...
  (tSimpl, _) <- foldrM
    (\(PTupleElement el) (expr, k) -> do
      pSimpl <- simplifyPattern
        fn
        recMode
        el
        (SimplifiedUnaryOp (OpTupleNth k tupleCount) letExpr)
        expr
      return (pSimpl, k + 1)
    )
    (expr, 0)
...
simplifyComplexExpression
  :: InferenceFn -> ComplexExpression -> Infer SimplifiedExpr
...
simplifyComplexExpression fn ast@(ECFor varName initExpr dir endExpr bodyExpr)
  = do
    ...
    addExprAnnot $ return $ SimplifiedLet
      varName
      (SimplifiedBinaryOp OpSame initSimplCheck endSimplCheck)
      bodySimpl
...
simplifyComplexExpression _ ECExportEnv =
  addExprAnnot $ return SimplifiedExportEnv
simplifyComplexExpression fn (ECTuple tuple) =
  addExprAnnot $ simplifyTuple fn tuple
simplifyComplexExpression fn (ECExpr expr) =
  addExprAnnot $ simplifyExpression fn expr
...
simplifyComplexExpression fn ast@(ECFunction bPip matchClauses) = do
  id <- freshIdent
  markTrace ast
  r <- simplifyComplexExpression fn $ ECFun (PatIdent id) [] $ ECMatch
    (ECExpr $ ExprVar id)
    bPip
    matchClauses
  unmarkTrace ast
...
simplifyComplexExpression fn (ECLetOperator recK opName patArgs letExpr expr) =
  ...
  --Ident $ getOperatorName opName
    addExprAnnot $ simplifyComplexExpression
      fn
      (ECLet recK
             (PatIdent $ Ident $ getOperatorName opName)
             patArgs
             TypeConstrEmpty
             letExpr
             expr
...
simplifyComplexExpression fn ast@(ECLet recK pat argsPats typeAnnot letExpr expr)
  = do
    markTrace ast
    expr        <- withTypeAnnot typeAnnot expr
    letSimpl    <- simplifyComplexExpression fn letExpr
    ...
    letSimplAcc <- foldrM
      (\pat expr -> do
        ...
        return $ SimplifiedFunction id s
      )
      letSimpl
      argsPats
...
simplifyTuple fn (DTuple firstElem elems) = do
  elemsSimpl <- foldM
    (\expr (DTupleElement el) -> do
      k <- simplifyExpression fn el
      return $ SimplifiedBinaryOp OpTupleCons k expr
    )
    (SimplifiedUnaryOp OpEmptyTuple SimplifiedSkip)
    ([firstElem] ++ elems)
...
simplifyList fn (DList elems) = do
  elemsSimpl <- foldM
    (\expr (ListElement el) -> do
      k <- simplifyComplexExpression fn el
      return $ SimplifiedBinaryOp OpCons k expr
    )
    (SimplifiedUnaryOp OpEmptyList SimplifiedSkip)
    elems
  addExprAnnot $ return $ elemsSimpl
...
simplifyExpression _ (ExprVar name) =
  addExprAnnot $ return $ SimplifiedVariable name
simplifyExpression _ (ExprConst (CInt val)) =
  addExprAnnot $ return $ SimplifiedConstInt val
simplifyExpression _ (ExprConst (CString val)) =
  addExprAnnot $ return $ SimplifiedConstString val
simplifyExpression _ (ExprConst (CBool CBTrue)) =
  addExprAnnot $ return $ SimplifiedConstBool True
simplifyExpression _ (ExprConst (CBool CBFalse)) =
  addExprAnnot $ return $ SimplifiedConstBool False
simplifyExpression fn (ExprList list) = addExprAnnot $ simplifyList fn list
simplifyExpression fn (ExprCompl expr) =
  addExprAnnot $ simplifyComplexExpression fn expr
simplifyExpression fn ast@(ExprCall exp firstArg restArgs) = do
  ...
  r      <- foldlM
    (\simpl argExp -> do
      ...
      return $ SimplifiedCall simpl argSimpl
    )
    (fnExpr)
    ([firstArg] ++ restArgs)
  unmarkTrace ast
...
simplifySimpleExpression
  :: InferenceFn -> SimpleExpression -> Infer SimplifiedExpr
simplifySimpleExpression fn (ESOp opName) =
  simplifyExpression fn $ ExprOp opName
simplifySimpleExpression fn (ESConst const) =
  simplifyExpression fn $ ExprConst const
simplifySimpleExpression fn (ESIdent name) =
  simplifyExpression fn $ ExprVar name
