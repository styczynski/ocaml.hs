...
execRecord ast@(DataRecord recordElements) = do
  proceed ast
  env <- ask
  (recordFields, recordEnv, fieldNames) <- foldM
    (\(res, env, names) (RecordElement fieldName exp) -> do
      (r, newEnv) <- shadow env $ local (\_ -> env) $ execComplexExpression exp
      return ((Map.insert fieldName r res), newEnv, [fieldName] ++ names)
    )
    (Map.empty, env, [])
    recordElements
  ...
  case recordTypeName of
    DInvalid ->
      raise
        $  "Invalid field name was specified for record type: "
        ++ (treeToStr ast)
...
execList ast@(DList listElements) = do
  proceed ast
  env                  <- ask
  (listItems, listEnv) <- foldM
    (\(res, env) (ListElement exp) -> do
      (r, newEnv) <- shadow env $ local (\_ -> env) $ execComplexExpression exp
      return ((res ++ [r]), newEnv)
    )
    ([], env)
...
execTuple ast@(DTuple firstElement tupleElements) = do
  proceed ast
  env                    <- ask
  (tupleItems, tupleEnv) <- foldM
    (\(res, env) (DTupleElement exp) -> do
...
execComplexExpression ast@(ECIf cond exp1 exp2) = do
  ...
  r                  <- if condVal
    then (shadow env $ local (\_ -> condEnv) $ execComplexExpression exp1)
    else (shadow env $ local (\_ -> condEnv) $ execComplexExpression exp2)
  unproceed
  return r
execComplexExpression ast@(ECWhile cond exp) = do
  ...
  if condVal
    then do
      (_, stEnv) <- shadow env $ local (\_ -> condEnv) $ execComplexExpression
        exp
      r <-
        (shadow env $ local (\_ -> stEnv) $ execComplexExpression
          (ECWhile cond exp)
        )
      unproceed
      return r
...
execComplexExpression ast@(ECFor name expVal1 dir expVal2 exp) = do
  ...
  (val2, env2) <-
    shadow env
    $   local (\_ -> env1)
    $   execComplexExpression expVal2
    >>= unpackInt
  env2i <- return $ setVariable name (RInt val1) env2
  f     <- case dir of
    ForDirTo -> if val1 <= val2
      then do
        (_, env3) <- shadow env $ local (\_ -> env2i) $ execComplexExpression
          exp
        shadow env $ local (\_ -> env3) $ execComplexExpression
          (ECFor name (ECExpr $ ExprConst $ CInt $ val1 + 1) dir expVal2 exp)
      else shadow env $ return $ (REmpty, env2)
    ForDirDownTo -> if val1 >= val2
      then do
        (_, env3) <- shadow env $ local (\_ -> env2i) $ execComplexExpression
          exp
        shadow env $ local (\_ -> env3) $ execComplexExpression
...
execComplexExpression ast@(ECMatch expr _ clauses) = do
  ...
  (matchEnv, matchRes) <- setMatchPatterns
    (map (\(MatchClause pat patExp) -> (pat, patExp)) clauses)
    expVal
    expEnv
  case matchRes of
...
execComplexExpression ast@(ECFunction bPip matchClauses) = do
  ...
  unproceed
  shadow env $ execComplexExpression $ ECFun (PatIdent id) [] $ ECMatch
    (ECExpr $ ExprVar id)
    bPip
    matchClauses
...
execComplexExpression ast@(ECLet LetRecNo pattern [] typeAnnot letExpr expr) =
  do
    proceed ast
    ...
    (val, valEnv)    <-
      shadow env $ local (\_ -> patEnv) $ (execComplexExpression expr)
    unproceed
    return $ (val, valEnv)
execComplexExpression ast@(ECLet r (PatIdent name) restPatterns typeAnnot letExpr expr)
  = do
    proceed ast
    ...
    fnBody    <- return $ \args -> do
      ...
      inEnv <- return
        $ if isRec r then importEnvRef newRef name inEnv env else inEnv
    ...
    (val, valEnv) <-
      shadow env
      $ local (createFunction name (Just newRef) (RFunSig argsCount) fnBody)
      $ (execComplexExpression expr)
    unproceed
    return $ (val, valEnv)
execComplexExpression ast@(ECLet r pattern [] typeAnnot letExpr expr) = do
  ...
  unproceed
  return $ (val, valEnv)
execComplexExpression ast@(ECLetOperator r opAny restPatterns letExpr expr) =
  do
    proceed ast
    validatePatterns (restPatterns)
    env              <- ask
    (id      , env ) <- return $ allocIdent env
    (valFnRef, env2) <- shadow env $ execComplexExpression
      (ECLet r
             (PatIdent id)
             restPatterns
             TypeConstrEmpty
    ...
    (val, valEnv) <-
      shadow env $ local (\_ -> defEnv) $ (execComplexExpression expr)
    unproceed
    return $ (val, valEnv)
execComplexExpression ast@(ECLet r (PatIdent name) restPatterns typeAnnot letExpr expr)
  = do
...
execComplexExpression ast@(ECFun pattern restPatterns bodyExpr) = do
  ...
  fnBody    <- return $ \args -> do
    inEnv  <- shadowE env ask
    patEnv <- local (\_ -> inEnv)
      $ setPatterns ([pattern] ++ restPatterns) args inEnv
...
execExpression ast@(ExprCall expr1 argFirst argsRest) = do
  ...
  (args, argsEnv) <- shadow env $ local (\_ -> fnEnv) $ foldM
    (\(res, env) exp -> do
