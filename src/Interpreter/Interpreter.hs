...
runAST tree env  = do
  ...
  result <- return (case r of
      ...
      Right ((res, env), InterpreterState { globalExportEnv = Nothing }) -> Executed res Types.TypeUnit env
      Right ((res, _), InterpreterState { globalExportEnv = (Just env) }) -> Executed res Types.TypeUnit env)
  return result
