...
annotateGlobalVariableTypeEnvironment env state name typeExpr =
  ...
  in
    case pTypeExpression ts of
      Ok tree -> do
        v <- runExceptT
          (runReaderT (runStateT (resolveTypeExpression tree) (state)) (env))
        case v of
...
instance (UnpackableValue a, PackableValue b) => PackableValue (VarargFun a b) where
  packVal (VarargFun innerFn) = do
    ...
    body <- return $ \args -> do
      bodyEnv                     <- ask
      (unpackedArgs, unpackedEnv) <- foldrM
        (\el (vals, env) -> do
          (val1, env1) <- local (\_ -> env) $ unpackVal el
          return $ ([val1] ++ vals, env1)
        )
        ([], bodyEnv)
        args
...
instance {-# OVERLAPS #-} (UnpackableValue a, UnpackableValue b, UnpackableValue c, UnpackableValue d, PackableValue e) => PackableValue (a -> b -> c -> d -> e) where
  packVal innerFn = do
    env  <- ask
    body <- return $ \[arg0, arg1, arg2, arg3] -> do
      bodyEnv      <- ask
